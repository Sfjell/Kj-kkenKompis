...
const Home: React.FC<HomeProps> = ({ onCapture, filters, onFilterChange, language, usageCount, isPro }) => {
  ...
  return (
    <div className="py-4 pb-12 max-w-4xl mx-auto">
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 gap-6 mb-12">
        <button 
          ...
          className="flex flex-row items-center justify-start p-8 bg-emerald-50 border-2 border-emerald-100 rounded-[32px] group hover:bg-emerald-100 transition-all hover:shadow-lg active:scale-95"
        >
          <div className="w-16 h-16 bg-emerald-500 text-white rounded-2xl flex items-center justify-center mr-6 group-hover:scale-110 transition-transform shadow-lg">
            <i className="fa-solid fa-camera text-2xl"></i>
          </div>
          <div className="text-left">
            <span className="block font-black text-xl text-emerald-900">{t.takePhoto}</span>
            <span className="text-emerald-700/60 font-bold text-sm">Gjenkjenn varer med AI</span>
          </div>
        </button>
        <button 
          onClick={() => fileInputRef.current?.click()}
          className="flex flex-row items-center justify-start p-8 bg-blue-50 border-2 border-blue-100 rounded-[32px] group hover:bg-blue-100 transition-all hover:shadow-lg active:scale-95"
        >
          <div className="w-16 h-16 bg-blue-500 text-white rounded-2xl flex items-center justify-center mr-6 group-hover:scale-110 transition-transform shadow-lg">
            <i className="fa-solid fa-upload text-2xl"></i>
          </div>
          <div className="text-left">
            <span className="block font-black text-xl text-blue-900">{t.upload}</span>
            <span className="text-blue-700/60 font-bold text-sm">Last opp fra galleri</span>
          </div>
        </button>
      </div>
      ...
      <section className="space-y-12">
        <div className="grid grid-cols-1 md:grid-cols-2 gap-12">
              ...
              <div className="flex flex-wrap gap-2">
                {CUISINES.map(c => (
                  <button 
                    ...
                    className={`px-6 py-2 rounded-2xl whitespace-nowrap font-bold transition-all border-2 ${
                      filters.cuisine === c 
              ...
              <div className="flex flex-wrap gap-2">
                {DIET_OPTIONS.map(d => (
                  <button 
                    ...
                    className={`px-5 py-2 rounded-2xl border-2 transition-all font-bold ${
          ...
          {/* Høyre side: Ernæring og Allergier */}
          <div className="space-y-10">
               ...
               <div className={`space-y-8 transition-opacity duration-300 ${filters.nutritionEnabled ? 'opacity-100' : 'opacity-30 pointer-events-none'}`}>
                  <div>
                    <div className="flex justify-between text-sm mb-3">
                      <span className="font-bold text-gray-500 uppercase tracking-widest text-xs">{t.maxCalories}</span>
                      <span className="font-black text-emerald-600">{filters.maxCalories} kcal</span>
                    </div>
                    <input 
                      type="range" min="200" max="1500" step="50"
                      className="w-full h-3 bg-gray-100 rounded-lg appearance-none cursor-pointer accent-emerald-500"
                  ...
                  <div>
                    <div className="flex justify-between text-sm mb-3">
                      <span className="font-bold text-gray-500 uppercase tracking-widest text-xs">{t.minProtein}</span>
